...
class PDBData:
    ...
    def import_pdb(self, filename):
        if not hasattr(self, 'mol'):
            self.mol = bb.Molecule()
        self.mol.import_pdb(filename)
    ...
    def atomselect(self, atoms, ignore_atoms=[]):
        if atoms == "*":
            _atoms = list(np.unique(self.mol.data["name"].values))
            for to_remove in ignore_atoms:
                if to_remove in _atoms:
                    _atoms.remove(to_remove)
        elif atoms == "no_hydrogen":
            _atoms = list(np.unique(self.mol.data["name"].values))    #all the atoms
            _plain_atoms = []
            for a in _atoms:
                if a in self.mol.knowledge['atomtype']:
                    _plain_atoms.append(self.mol.knowledge['atomtype'][a])
                elif a[:-1] in self.mol.knowledge['atomtype']:
                    _plain_atoms.append(self.mol.knowledge['atomtype'][a[:-1]])
                else:
                    _plain_atoms.append(self.mol.knowledge['atomtype'][a]) # if above failed just raise the keyerror
            _atoms = [atom for atom, element in zip(_atoms, _plain_atoms) if element != 'H']
        else:
            _atoms = [_a for _a in atoms if _a not in ignore_atoms]

        _, self._idxs = self.mol.atomselect("*", "*", _atoms, get_index=True)
        self.mol = self.mol.get_subset(self._idxs)

    def prepare_dataset(self, ):
        if not hasattr(self, 'dataset'):
            assert hasattr(self, 'mol'), 'You need to call import_pdb before preparing the dataset'
            self.dataset = self.mol.coordinates
    ...
    def get_atominfo(self):
        if not hasattr(self, 'atominfo'):
            assert hasattr(self, 'mol'), 'You need to call import_pdb before getting atom info'
            self.atominfo = self.mol.get_data(columns=['name', 'resname', 'resid'])
        return self.atominfo

    def frame(self):
        M = bb.Molecule()
        M.coordinates = self.mol.coordinates[0]
        M.data = self.mol.data
        M.data['index'] = np.arange(self.mol.coordinates.shape[1])
        M.current = 0
