...
class MolearnAnalysis:
    ...
    def get_error_padding(
        self, key: str, std: list[float], mean: list[float], align=True
    ):
        """
        Calculate the reconstruction error of a dataset encoded and decoded by a trained neural network. Same as get_error but designed to work with zero-padding

        :param str key: key pointing to a dataset previously loaded with :func:`set_dataset <molearn.analysis.MolearnAnalysis.set_dataset>`
        :param std float: list with the standard deviation of the single loaded dataset before merging with padding - needs to have as many values as frames
        :param mean float: list with the mean of the single loaded dataset before merging with padding - needs to have as many values as frames
        :param bool align: if True, the RMSD will be calculated by finding the optimal alignment between structures
        :return: 1D array containing the RMSD between input structures and their encoded-decoded counterparts
        """
        dataset = self.get_dataset(key)
        ...
        m = deepcopy(self.mol)
        for i in range(dataset.shape[0]):
            # which part of the dataset is masked
            padding_mask = dataset[i] != 0.0
            # masks padding and any eliminates the need to clip the prediction to be the original size again
            ground_truth = dataset[i][padding_mask].reshape((3, -1))
            prediction = decoded[i][:, : dataset.shape[2]][padding_mask].reshape(
                (3, -1)
            )

            crd_ref = (
                as_numpy(ground_truth.permute(1, 0).unsqueeze(0)) * std[i] + mean[i]
            )
            crd_mdl = as_numpy(prediction.permute(1, 0).unsqueeze(0) * std[i] + mean[i])
            # use Molecule Biobox class to calculate RMSD
            if align:
                m.coordinates = deepcopy(crd_ref)
                m.set_current(0)
