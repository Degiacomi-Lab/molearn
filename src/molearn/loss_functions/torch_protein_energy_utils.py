...
def amber_card_type_6(f_in, torsion_factor, torsion_barrier, torsion_phase,
    ...
    # section 6 torsion / proper dihedral
    for line in f_in:
        if line == '\n' or line.strip() == '':
            break
        atom1 = line[0:2].strip()
...
def amber_card_type_9(f_in, other_parameters):
    # section 9 equivalencing atom symbols for non-bonded 6-12 potential parameters
    for line in f_in:
        if line=='\n' or line.strip()=='':
            break
        contents = line.split()
...
def get_convolutions(dataset, pdb_atom_names,
    ...
    if fix_charmm_residues:
        ...
        # if any HIS are remaining it does not matter which because the H is dealt with above
    ...
    # we do not want barrier/factor to return nan so set factor to 1 by default
    torsion_para[:,0,:]=1.0
...
def get_conv_pad_res(dataset, pdb_atom_names,
    ...
    if len(dataset.shape) != 3:
        raise Exception(f'967 dataset frame here should be of shape [R, M, 3] not {str(dataset.shape)}')
...
#    ##################### Angles/1-3 #####################
    ...
    bond_idxs = torch.as_tensor(bond_idxs_)
